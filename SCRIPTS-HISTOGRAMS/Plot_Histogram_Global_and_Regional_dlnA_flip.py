...
def SYMETRIC_BIN(data):
    ...
    #nbins_symetric = np.arange(-nbins, nbins + 1, 0.1)
    #nbins_symetric = np.arange(-nbins + 0.5, nbins + 0.5, 0.1)
    nbins_symetric = np.arange(-nbins + 0.05, nbins + 0.05, 0.1)
    #nbins_symetric = np.arange(-nbins, nbins, 1)
...
with open('configZRT_dlnA.yaml') as Fym:
...
#open a file for regional and global data: min, max, mean, sigma, N
Fdata = open("ComputeData.dat", "w")
#ll    = "%s  %s   %s   %s   %s    %s   %s    %s"%('category', 'component', 'BP', 'min', 'max', 'mean', 'sigma','nwindows')

ll   = "{:5s} {:20s} {:3s} {:6s} {:6s} {:6s} {:6s} {:9s}".format('Type', 'Category', 'Component', 'min', 'max', 'mean', 'sigma','nwindows')
##################
Fdata.write("%s\n"%(ll))
#for m in modes:
...
for mode in modes:
        ...
        for station in stations:
            ...
            for component in windows[station].keys():
                ...
                for cwind in windows[station][component]:
                    ...
                    value              = float(cwind["cc_shift_in_seconds"])
                    #Save the data into the Global dictionary 
                    DicGlob[win_type][comp].append(dlnA)
                    ...
                    if(event_name in REG_EVENTS and station in REG_STATIONS):
                        DicReg[win_type][comp].append(dlnA)
    ...
    #figname  = "ZRT_Histogram_%s_Delta_dlnA.pdf"%(mode.replace("#","_"))
    figname  = "ZRT_Histogram_%s_Delta_dlnA.png"%(mode.replace("#","_"))
    #Grid the figure
    ...
    for comp in comps:
        ...
        #Write into the file
        ll_R = "{:5s} {:20s} {:3s} {:6.2f} {:6.2f} {:6.2f} {:6.2f} {:9d}".format('REG',  mode, comp, min(data_trim_R), max(data_trim_R), mu_R, sigma_R, data_trim_R.size)
        ll_G = "{:5s} {:20s} {:3s} {:6.2f} {:6.2f} {:6.2f} {:6.2f} {:9d}".format('GLOB', mode, comp, min(data_trim_G), max(data_trim_G), mu_G, sigma_G, data_trim_G.size)
        #Write into the file
        Fdata.write("%s\n"%(ll_R))
        Fdata.write("%s\n"%(ll_G))
        ...
        if density:
            ...
            YY_max_G      = float('%.2f'%(N_Limit_G))
            YY_max_R      = float('%.2f'%(N_Limit_R))
            #YY_max        = 0.86 * max(YY_max_G, YY_max_R)
            YY_max        = 8.0 * max(YY_max_G, YY_max_R)
            #####################
            if(comp=="Z"):
                    ####################################
                    ...
                    Scatter_G(ax1, P_G_Y, data_trim_G,AxiVis[mode]['gc']) 
                    ...
                    Scatter_R(ax2, P_R_Y, data_trim_R,AxiVis[mode]['rc'])
                    ...
                    #ax2.set_xlabel(r'$\Delta T$'+'(s)', fontsize=fontsize_Label, color='k', labelpad=10, fontweight= 'bold')
                    ...
                    ax1.text(t2,ymax/6.0,'(%s)'%(FigLab[mode][0]),horizontalalignment='center',verticalalignment='center',alpha=1.0,fontweight='bold',fontsize=fsize_abc)
                    #Axis visibility
                    ...
                    #Write a lettter in Figure
                        ax2.set_xlabel(r'$\Delta A/A$', fontsize=fontsize_Label, color='k', labelpad=10, fontweight= 'bold')
##################################################################################################
            if(comp=="R"):
                    ...
                    ax1.text(t2,ymax/6.0,'(%s)'%(FigLab[mode][1]),horizontalalignment='center',verticalalignment='center',alpha=1.0,fontweight='bold',fontsize=fsize_abc)
                    #Axis visibility
            ...
            if(comp=="T"):
                    ...
                    ax1.text(t2, ymax/6.0,'(%s)'%(FigLab[mode][2]),horizontalalignment='center',verticalalignment='center',alpha=1.0,fontweight='bold',fontsize=fsize_abc)
                    #Font of the y-axis
                    ax1.yaxis.set_tick_params(labelsize=fontsize_Label)
                    #Font x-axis
                    ax2.xaxis.set_tick_params(labelsize=fontsize_Label)
